...
export default function SectionHeader({ eyebrow, title, description, align = 'center' }) {
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col gap-4 mb-12 sm:mb-16 ${textAlign}`}
    >
      {eyebrow && (
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass border border-blue-500/20 self-start mx-auto w-fit">
          ...
          <span className="text-blue-400 text-xs sm:text-sm font-medium tracking-wide uppercase">{eyebrow}</span>
        </div>
      )}
      ...
      {description && (
        <p className="text-slate-400 text-sm sm:text-base md:text-lg max-w-2xl leading-relaxed break-words">
          {description}
        </p>
      )}
      <div className={`flex items-center gap-3 mt-2 ${align === 'center' ? 'justify-center' : ''}`}>
        ...
        <div className="h-px w-8 sm:w-12 bg-gradient-to-l from-purple-500 to-transparent" />
      </div>
    </motion.div>
  )
}
